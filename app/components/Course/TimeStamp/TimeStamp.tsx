...
function TimeStamp({ course, isUserRegisteredToThisCourse }: { course: singleCourseType; isUserRegisteredToThisCourse: boolean }) {
  if (course.price > 0 || isUserRegisteredToThisCourse === true) {
    return "";
  }
  ...
  return (
    <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row gap-y-4 gap-x-5 flex-wrap justify-between items-center text-center sm:text-right p-4 xl:h-16 bg-gray-100 lg:bg-white dark:bg-gray-900 lg:dark:bg-darker rounded-lg mt-5 lg:mt-0">
      <span className="font-DanaDemiBold sm:text-xl lg:text-center xl:text-right text-red-500">100% پیشنهاد شگفت انگیز</span>
