    ...
    <div className="bg-white dark:bg-darker rounded-xl p-[18px] sm:p-5">
      ...
      <div className="mt-3.5 sm:mt-5">
        <div className="flex items-center justify-between font-DanaDemiBold text-sm sm:text-base mb-3">
          ...
          <span>{course.status === "presell" ? 0 : 57}%</span>
        </div>
        {/* <div className={`${course.status === "presell" ? "w-0" : "w-full"} h-3 rounded-full bg-primary`}></div> */}
        <div className="w-full h-3 rounded-full bg-primary/20" dir="ltr">
          <div className={`h-full rounded-full bg-primary ${course.status === "presell" ? "w-0" : "w-[57%]"}`}></div>
        </div>
      </div>
    </div>
  );
