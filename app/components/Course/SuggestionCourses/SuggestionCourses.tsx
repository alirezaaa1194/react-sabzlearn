...
function SuggestionCourses({ course }: { course: singleCourseType }) {
  ...
  return (
    <div className="hidden lg:block bg-white dark:bg-darker rounded-xl p-[18px] sm:p-5 mt-8">
      ...
      <div className="space-y-4 md:space-y-5">{suggestionCourses.length ? suggestionCourses.slice(suggestionCourses.length - 5).map((suggestionCourse: courseType) => <SuggestionCourseBox suggestionCourse={suggestionCourse} />) : <h3 className="font-DanaMedium text-center">دوره ای وجود ندارد.</h3>}</div>
    </div>
  );
}
