...
function MobileSort() {
  ...
  return (
    <>
      ...
      <Button onClick={() => setIsOpenSortBox(true)} className="flex w-full md:hidden items-center !mb-8 dark:text-white dark:bg-darker rounded-lg h-[42px]">
      ...
      <div className={`block lg:hidden fixed right-0 z-[1000000] transition-all rounded-t-2xl w-full ${isOpenSortBox ? "bottom-0 visible opacity-100" : "-bottom-full invisible opacity-0"}`}>
        ...
        <div className="bg-white dark:bg-darker w-full flex flex-col px-5">
          ...
          <Button onClick={() => changeSortHandler("newest")} className={`px-0 flex items-center justify-between h-[68px] w-full bg-transparent text-white active:text-primary font-DanaMedium text-base rounded-none outline-none border-b border-b-gray-100 dark:border-b-dark ${searchParams.get("sort") === "newest" ? "active" : ""}`}>
            <span className="">جدیدترین</span>
            <CheckCircleIcon className={`size-6 text-primary ${searchParams.get("sort") === "newest" ? "block" : "hidden"}`} />
          </Button>
          <Button onClick={() => changeSortHandler("oldest")} className={`px-0 flex items-center justify-between h-[68px] w-full bg-transparent text-white active:text-primary font-DanaMedium text-base rounded-none outline-none border-b border-b-gray-100 dark:border-b-dark ${searchParams.get("sort") === "oldest" ? "active" : ""}`}>
            <span className="">قدیمی ترین</span>
            <CheckCircleIcon className={`size-6 text-primary ${searchParams.get("sort") === "oldest" ? "block" : "hidden"}`} />
          </Button>
          <Button onClick={() => changeSortHandler("popular")} className={`px-0 flex items-center justify-between h-[68px] w-full bg-transparent text-white active:text-primary font-DanaMedium text-base rounded-none outline-none border-none ${searchParams.get("sort") === "popular" ? "active" : ""}`}>
