...
function FilterDrawer() {
  const [isOpenFilterDrawer, setIsOpenFilterDrawer] = useState<boolean>(true);
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 z-[10000000000] w-svw h-svh bg-white dark:bg-darker  flex flex-col lg:hidden transition-all ${isOpenFilterDrawer ? "visible opacity-100" : "invisible opacity-0"}`}>
        ...
        <FilterSwitch removeAllFilter={removeAllFilter} setRemoveAllFilter={setRemoveAllFilter} setIsOpenFilterDrawer={setIsOpenFilterDrawer} />
      </div>
    </>
  );
}
