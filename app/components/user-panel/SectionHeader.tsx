...
function SectionHeader({ title, linkTitle, linkAdress, elem }: { title: string; linkTitle?: string; linkAdress?: string; elem?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between bg-white dark:bg-darker h-12 md:h-[72px] pl-2 md:pl-6 mb-5 rounded">
      <div className="flex items-center gap-x-3 md:gap-x-6 h-full">
        ...
        <span className="text-sky-500 text-sm md:text-lg font-DanaDemiBold md:font-DanaDemiBold">{title}</span>
      </div>
      {linkTitle ? (
