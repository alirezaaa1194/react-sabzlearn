...
function EditCourseForm({ course, categories }: { course: singleCourseType; categories: categoryType[] }) {
  ...
  const schema = yup
    .object({
      name: yup.string().min(3, "نام دوره باید حداقل ۳ کاراکتر باشد").max(25, "نام دوره نباید بیشتر از 25 کاراکتر باشد").required("لطفاً نام دوره را وارد کنید"),
      ...
      shortName: yup.string().min(3, "نام کوتاه دوره باید حداقل ۳ کاراکتر باشد").max(20, "نام کوتاه دوره نباید بیشتر از 20 کاراکتر باشد").required("لطفاً نام کوتاه را وارد کنید"),
  ...
  const onSubmit = (data: any) => {
    ...
    fetcher.submit(formData, { method: "POST", encType: "multipart/form-data", action: "/EditCourse" });

    if (fetcher?.data?.success === false) {
      showToast("خطا", "خطای غیر منتظره رخ داده است", "error");
    } else {
      showToast("موفق", "دوره با موفقیت ویرایش شد", "success");
      handleReset();
    }
  };

  methods.setValue("name", course.name!);
  methods.setValue("price", String(course.price)!);
  methods.setValue("shortName", course.shortName!);
  methods.setValue("support", course.support!);
  methods.setValue("category", course.categoryID._id!);
  methods.setValue("status", course.status!);

  useEffect(() => {
    setTimeout(() => {
      if (editorRef?.current) {
        editorRef?.current.setData(course.description);
      }
    }, 1000);
  }, [course, categories]);
    ...
    <FormProvider {...methods}>
      <form className="grid grid-cols-1 md:grid-cols-2 gap-4 gap-y-7" onSubmit={methods.handleSubmit(onSubmit)}>
          ...
          <Controller
            ...
            render={({ field, fieldState: { error } }) => (
              <Select
                ...
                defaultSelectedKeys={[course.categoryID._id]}
        ...
        <Button type="submit" className="bg-secondary hover:bg-secondary-hover transition-colors rounded-lg text-white font-DanaMedium text-base w-full md:w-fit md:px-10 md:col-span-2 ms-auto mt-5">
          {fetcher.state === "loading" ? <Spinner color="white" size="sm" /> : "ویرایش دوره"}
        </Button>
      </form>
    </FormProvider>
  );
