...
function AddUserForm({ users }: { users: userType[] }) {
  ...
  const onSubmit = (data: any) => {
    ...
    fetcher.submit({ newUserInfo: JSON.stringify(newUserInfo) }, { method: "POST", action: "/RegisterUser" });

    if (fetcher?.data?.success === false) {
      showToast("خطا", "خطای غیر منتظره رخ داده است", "error");
    } else {
      showToast("موفق", "کاربر با موفقیت افزوده شد", "success");
      handleReset();
    }
  };
...
"
        >
          <Button type="submit" className="bg-secondary hover:bg-secondary-hover transition-colors rounded-lg text-white font-DanaMedium text-base md:px-10 flex-grow md:grow-0">
            {fetcher.state === "loading" ? <Spinner color="white" size="sm" /> : "افزودن کاربر"}
          </Button>
        </div>
      </form>
    </FormProvider>
