...
function AddArticleForm({ articles, categories }: { articles: articleType[]; categories: categoryType[] }) {
  ...
  const schema = yup
    .object({
      title: yup.string().min(3, "عنوان مقاله باید حداقل ۳ کاراکتر باشد").max(90, "عنوان مقاله نباید بیشتر از 90 کاراکتر باشد").required("لطفاً عنوان مقاله را وارد کنید"),
      shortName: yup.string().min(3, "نام کوتاه مقاله باید حداقل ۳ کاراکتر باشد").max(50, "نام کوتاه مقاله نباید بیشتر از 50 کاراکتر باشد").required("لطفاً نام کوتاه را وارد کنید"),
      description: yup.string().min(30, "توضیحات مقاله باید حداقل 30 کاراکتر باشد").max(400, "توضیحات مقاله نباید بیشتر از 400 کاراکتر باشد").required("لطفاً توضیحات مقاله را وارد کنید"),
  ...
  return (
    <FormProvider {...methods}>
      <form className="grid grid-cols-1 md:grid-cols-2 gap-4 gap-y-7" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="col-span-1 flex flex-col">
          <CustomInput name="shortName" label="نام کوتاه مقاله" placeholder="لطفا نام کوتاه مقاله را وارد کنید" />
        </div>
